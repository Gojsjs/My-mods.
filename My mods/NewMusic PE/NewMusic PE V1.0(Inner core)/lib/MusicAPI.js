...
function PlaySong(name, number, format) {
	try {
var song = parseInt(Math.random() * number);
		var media = null;
		for(var i in SoundPlayers){
			var SP = SoundPlayers[i];
			if(!SP.isPlaying()) {
		...
		media.reset();
		media.setDataSource(__dir__ + "res/sounds/"+name+song+format);
		media.prepare();
		media.start();
		return media;
	} catch(err) {}
...
registerAPIUnit("PlaySong", PlaySong);
